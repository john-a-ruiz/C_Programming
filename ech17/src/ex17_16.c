/*
 * modify invent.c (section 16.3) so the print command calls qsort to sort the array before printing it
 */
...
int part_sort(const void *p, const void *q);
...
/*
 * print:	Prints a listing of all parts in the database by inventory number using qsort,
...
//		inventory[i] = ordered[i];
	...
	qsort(inventory, num_parts, sizeof(struct part), part_sort);
